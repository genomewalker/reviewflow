...
const API_BASE = 'http://localhost:3001';
        ...
        function renderProjectManager() {
            ...
            grid.innerHTML = papers.map(paper => {
                ...
                return `
                    <div class="pm-paper-card ${isSelected ? 'selected' : ''}" onclick="selectPaperFromManager('${escapeHtml(paper.id)}')">
                        <div class="pm-paper-header">
                            <h3 class="pm-paper-title">${escapeHtml(paper.title)}</h3>
                            <button onclick="event.stopPropagation(); deletePaperFromManager('${escapeHtml(paper.id)}')" class="pm-paper-delete" title="Move to trash">
                                <i class="fas fa-trash-alt"></i>
                            </button>
                        </div>
                        ${paper.journal ? `<p class="pm-paper-journal">${escapeHtml(paper.journal)}</p>` : ''}
                        ${paper.authors ? `<p class="pm-paper-authors">${escapeHtml(paper.authors)}</p>` : '<div style="margin-bottom: var(--sp-3);"></div>'}
                        <div class="pm-paper-footer">
                            <div class="pm-progress-container">
                                <div class="pm-progress-bar">
                                    <div class="pm-progress-fill ${pct === 100 ? 'complete' : pct > 0 ? 'partial' : 'empty'}" style="width: ${pct}%"></div>
                                </div>
                                <span class="pm-progress-text">${done}/${total}</span>
                            </div>
                            ${isSelected ? '<span class="pm-selected-badge">Open</span>' : ''}
                        </div>
                    </div>
                `;
            }).join('');
        ...
        function showPapersTab() {
            // Update tab styles
            document.getElementById('tab-papers').className = 'pm-tab active';
            document.getElementById('tab-trash').className = 'pm-tab';
        ...
        function showTrashTab() {
            // Update tab styles
            document.getElementById('tab-papers').className = 'pm-tab';
            document.getElementById('tab-trash').className = 'pm-tab active';
        ...
        function renderTrash() {
            ...
            grid.innerHTML = trashedPapers.map(paper => {
                ...
                return `
                    <div class="pm-trash-card">
                        <div class="pm-paper-header">
                            <h3 class="pm-paper-title">${escapeHtml(paper.title)}</h3>
                        </div>
                        ${paper.journal ? `<p class="pm-paper-journal">${escapeHtml(paper.journal)}</p>` : ''}
                        <p class="pm-trash-date">Deleted ${escapeHtml(deletedDate)}</p>
                        <div class="pm-trash-actions">
                            <button onclick="restorePaper('${escapeHtml(paper.id)}')" class="pm-restore-btn">Restore</button>
                            <button onclick="permanentlyDeletePaper('${escapeHtml(paper.id)}')" class="pm-delete-forever-btn" title="Delete permanently">
                                <i class="fas fa-trash-alt"></i>
                            </button>
                        </div>
                    </div>
        ...
        function updateCategoryFilesList(category) {
            ...
            filesDiv.innerHTML = categoryFiles[category].map((f, idx) => `
                <div class="pm-file-item">
                    ...
                    <button onclick="event.stopPropagation(); removeCategoryFile('${category}', ${idx})" class="pm-file-remove">
...
Return ONLY JSON:`;
                ...
                showOpenCodeLoading('metadata-extraction', 'Extracting paper metadata...');
                ...
                if (response.ok) {
                    ...
                    if (jsonMatch) {
                        ...
                        if (metadata.title || metadata.authors || metadata.journal || metadata.field) {
                            ...
                            hideOpenCodeLoading('metadata-extraction', { success: true, message: 'Metadata extracted' });
                        } else {
                            hideOpenCodeLoading('metadata-extraction');
                        }
                    } else {
                        hideOpenCodeLoading('metadata-extraction');
                    }
                } else {
                    hideOpenCodeLoading('metadata-extraction', { success: false, message: 'Failed to extract metadata' });
                }
            } catch (e) {
                console.error('Error extracting metadata:', e);
                hideOpenCodeLoading('metadata-extraction', { success: false, message: 'Error extracting metadata' });
            }
        }
